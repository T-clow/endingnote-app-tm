...
見やすい大きなアイコンで、ユーザーの状況に応じて変遷先を変更する仕組みを採用  
- 未ログイン状態 → ログインへ
- データ未登録時 → 新規登録へ
- データ登録済み → 詳細および一覧ページへ

これによりユーザーは現在の自身の状況を意識せずとも、目的のページへストレスフルで的確に移動できます。
...
このアプリではいつ亡くなられた時にいくら残せるのかというものを、  
可視化してわかりやすくすることで、その後の葬儀の計画や予算の立案を支援します。  
2000万円の保険に入っていたつもりが、いざ亡くなると逓減型の保険で保険金は100万円でした。  
...
視認性の高いアイコンを表示、ユーザーの状況に応じて変遷先が変わるようにしてあります。  
- 未ログインならログイン画面へ
- データ登録がなければ新規登録へ
- データがあれば詳細画面、または一覧画面
